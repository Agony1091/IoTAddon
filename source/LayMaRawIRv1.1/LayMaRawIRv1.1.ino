...
// Temporary variables
int giatri = 0;
int t_nhan=50;             // Thoi gian nhan nut
int t_cho=2000;            // Thoi gia cho
   ...
   // Dieu chinh nut gio, Gio hien thi tren remote ban dau la tu dong
       for(wind=3;wind>=0;wind--)                                  //các chế độ tốc độ gió khác nhau, toc do gio tang dan
        { 
          ...
          delay_ms(t_nhan);                                          //hien thi trang thai
          ...
          if (nhietdo==16&&test==1){  
            for(nhietdo=17;nhietdo<31;nhietdo++){
          ...
          if (nhietdo==30&&test == 1 ){
                    for(int j=29;j>=16;j--){
                        ...
                        delay_ms(t_nhan);                                     
                        digitalWrite(opto5nutDOWN, LOW);                //nhả UP + hiển thị mã raw
                        delay_ms(t_nhan);
                        digitalWrite(opto1nutONOFF, HIGH);                      //nhấn OFF
                        delay_ms(t_nhan);
                        digitalWrite(opto1nutONOFF, LOW);                       //nhả OFF
                        ...
                        digitalWrite(opto1nutONOFF, HIGH);                      //nhấn ON
                        delay_ms(t_nhan); 
                        digitalWrite(opto1nutONOFF, LOW);             //nhả ON + hiển thị mã raw
                        delay_ms(t_cho);
                        ONOFF = 0;
                    }
                   test = 0;
